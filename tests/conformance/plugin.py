"""Shared fake-adapter factories and suite inputs for conformance suites.

A real adapter's test module overrides the fixture by declaring
``pytest_plugins = ["conformance.plugin"]``, importing the suite's test
functions and providing its own fixtures with the same names.  Only network
I/O is mocked in those modules.

Suite inputs that depend on the adapter (a plan the engine must accept or
reject, an evaluation input the harness accepts, the target ``verify`` runs
on) are fixtures too, so the same assertions hold for fakes and real adapters.
"""
...
_HEADS = {"num_attention_heads": "32", "num_kv_heads": "8"}


@pytest.fixture()
def valid_plan() -> DeploymentPlan:
    """TP=2 divides 32 attention heads and 8 KV heads."""
    return DeploymentPlan(tensor_parallel=2, resource_allocation=dict(_HEADS))


@pytest.fixture()
def incompatible_plan() -> DeploymentPlan:
    """TP=3 divides neither 32 attention heads nor 8 KV heads."""
    return DeploymentPlan(tensor_parallel=3, resource_allocation=dict(_HEADS))


@pytest.fixture()
def engine_verify_target() -> Any:
    """The target ``verify`` reads from; the fake engine needs none."""
    return None
...
def evaluation_input() -> dict[str, Any]:
    """A protocol input the adapter accepts; executing it yields one failed case."""
    return {"harness": "inspect_ai", "scorer": "exact_match"}


@pytest.fixture()
def unknown_evaluation_input() -> dict[str, Any]:
    return {"harness": "nonexistent_harness"}
