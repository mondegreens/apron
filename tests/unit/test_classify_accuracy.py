...
class TestEngineInit:
    def test_unsupported_task(self) -> None:
        assert _classify("Unsupported task: 'summarize'") == "engine_init"

    def test_lora_not_enabled(self) -> None:
        assert _classify("LoRA is not enabled. Use --enable-lora to enable LoRA.") == "engine_init"
    ...
    def test_device_type_infer_failure(self) -> None:
        assert _classify("Failed to infer device type") == "engine_init"
